            ...
            if ((sw_version_major_query is not None) and (sw_version_minor_query is not None)):
                ...
                # Monitoring frame.
                    ...
                    if (sw_version_major >= 7):
                        ...
                        storage_voltage_volts = float(storage_voltage_mv / 1000.0)
                        # MCU temperature.
                ...
                # Weather data frame.
                    ...
                    if (sw_version_major >= 8):
                        ...
                        if (sunshine_light_unit == 0):
                            ...
                            sunshine_light = float(sunshine_light_value * 10.0)
                        # Sunshine UV index.
                        sunshine_uv_index_raw = ((int(ul_payload[8:11], 16) >> 2) & 0x7F)
                        sunshine_uv_index = float(sunshine_uv_index_raw / 10.0)
                        ...
                        if (len(ul_payload) == (2 * ul_payload_size_weather_cm)):
                            ...
                            # Wind direction.
                            wind_direction_average_raw = ((int(ul_payload[19:22], 16) >> 1) & 0x01FF)
                        ...
                        humidity_error_value = METEOFOX_ERROR_VALUE_HUMIDITY_V2
                        # Sunshine light.
                        sunshine_light_raw = int(ul_payload[5:7], 16)
                        sunshine_light = float(sunshine_light_raw)
                        sunshine_light_error_value = METEOFOX_ERROR_VALUE_SUNSHINE_LIGHT_V2
                        # Sunshine UV index.
                        sunshine_uv_index_raw = int(ul_payload[7:8], 16)
                        sunshine_uv_index = float(sunshine_uv_index_raw)
                        ...
                        humidity_error_value = METEOFOX_ERROR_VALUE_HUMIDITY_V1
                        # Sunshine light.
                        sunshine_light_raw = int(ul_payload[4:6], 16)
                        sunshine_light = float(sunshine_light_raw)
                        sunshine_light_error_value = METEOFOX_ERROR_VALUE_SUNSHINE_LIGHT_V1
                        # Sunshine UV index.
                        sunshine_uv_index_raw = int(ul_payload[6:8], 16)
                        sunshine_uv_index = float(sunshine_uv_index_raw)
                    ...
                    record.add_field(humidity_percent, humidity_error_value, DATABASE_FIELD_HUMIDITY, float(humidity_percent))
                    record.add_field(sunshine_light_raw, sunshine_light_error_value, DATABASE_FIELD_SUNSHINE_LIGHT, float(sunshine_light))
                    record.add_field(sunshine_uv_index_raw, sunshine_uv_index_error_value, DATABASE_FIELD_SUNSHINE_UV_INDEX, float(sunshine_uv_index))
