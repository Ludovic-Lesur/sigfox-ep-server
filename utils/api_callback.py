...
CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPES_LIST = "sigfox_device_types_list"
CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPE_NAME = "name"
CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPE_ID = "id"
...
SIGFOX_CLOUD_API_JSON_KEY_FIELD = "fields"
...
SIGFOX_CLOUD_API_JSON_KEY_EP_ID = "id"
SIGFOX_CLOUD_API_JSON_KEY_MESSAGE_COUNTER = "seqNumber"
SIGFOX_CLOUD_API_JSON_KEY_OOB = "oob"
...
def API_CALLBACK_read_credentials_file():
    ...
    try:
        ...
        # Check headers.
        if ((CREDENTIALS_JSON_KEY_SIGFOX_EP_SERVER_ADDRESS not in sigfox_api_callback_credentials_json) or
            (CREDENTIALS_JSON_KEY_SIGFOX_CLOUD_USER not in sigfox_api_callback_credentials_json) or
            (CREDENTIALS_JSON_KEY_SIGFOX_CLOUD_PASSWORD not in sigfox_api_callback_credentials_json) or
            (CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPES_LIST not in sigfox_api_callback_credentials_json)):
...
def API_CALLBACK_update_sigfox_ep_server_address(sigfox_ep_server_name):
    ...
    if (sigfox_ep_server_name == API_CALLBACK_SERVER_ADDRESS_LOCAL):
        ...
        sigfox_ep_server_address = sigfox_api_callback_credentials_json[CREDENTIALS_JSON_KEY_SIGFOX_EP_SERVER_ADDRESS] + ":" + str(SIGFOX_EP_SERVER_HTTP_PORT)
...
def API_CALLBACK_get_device_type_id(device_type_name):
    ...
    # Device types loop.
    try:
        device_types_list = sigfox_api_callback_credentials_json[CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPES_LIST]
        for device_type_idx, device_type in enumerate(device_types_list):
            # Check headers.
            if ((CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPE_NAME not in device_type) or
                (CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPE_ID not in device_type)):
                print("[API CALLBACK] * ERROR: missing header in device type " + str(device_type_idx) + " of credentials file")
            else:
                if (device_type[CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPE_NAME] == device_type_name):
                    device_type_id = device_type[CREDENTIALS_JSON_KEY_SIGFOX_DEVICE_TYPE_ID]
                    status = True
                    break
    except Exception as expection_message:
        print(expection_message)
...
def API_CALLBACK_send_sigfox_cloud_api_request(request, parameters):
    ...
    sigfox_cloud_user = sigfox_api_callback_credentials_json[CREDENTIALS_JSON_KEY_SIGFOX_CLOUD_USER]
    sigfox_cloud_password = sigfox_api_callback_credentials_json[CREDENTIALS_JSON_KEY_SIGFOX_CLOUD_PASSWORD]
...
devices_list_request_parameters = {SIGFOX_CLOUD_API_JSON_KEY_DEVICE_TYPE_ID: device_type_id}
...
devices_list = sigfox_cloud_api_json.get(SIGFOX_CLOUD_API_JSON_KEY_DATA)
# Devices loop.
for device in devices_list:
    # Get ID.
    ep_id = device.get(SIGFOX_CLOUD_API_JSON_KEY_EP_ID)
    # Build request.
    ...
    if (int(timestamp_start_epoch_ms) == 0) and (int(timestamp_stop_epoch_ms) == 0):
        ...
        messages_list_request_parameters = {
            SIGFOX_CLOUD_API_JSON_KEY_FIELD: SIGFOX_CLOUD_API_JSON_FIELDS,
            SIGFOX_CLOUD_API_JSON_KEY_OOB: SIGFOX_CALLBACK_JSON_TRUE
        }
    else:
