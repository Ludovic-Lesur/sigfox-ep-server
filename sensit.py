...
__SENSIT_CONFIGURATION_DATA_LENGTH_BYTES = 12
...
def SENSIT_fill_data_base(timestamp, sigfox_ep_id, data) :
    # Init JSON object.
    json_body = []
    # Get version.
    sensit_version = __SENSIT_get_version(sigfox_ep_id)
    # Monitoring frame.
    if (len(data) == (2 * __SENSIT_MONITORING_DATA_LENGTH_BYTES)) or (len(data) == (2 * __SENSIT_CONFIGURATION_DATA_LENGTH_BYTES)) :
        # Parse fields.
        vbat_mv = "error"
        mode = "error"
        tamb_degrees = "error"
        ...
        if (sensit_version.find("V3") >= 0) :
            vbat_mv = (((int(data[0:2], 16) >> 3) & 0x1F) * 50) + 2700
            mode = ((int(data[2:4], 16) >> 3) & 0x0F)
        else :
            vbat_mv = ((((int(data[0:2], 16) >> 3) & 0x10) + (int(data[2:4], 16) & 0x0F)) * 50) + 2700
            mode = (int(data[0:2], 16) & 0x07)
        # Check mode.
        if (mode == 0x01):
            if (sensit_version.find("V3") >= 0):
                tamb_degrees = ((((int(data[2:4], 16) << 8) & 0x300) + (int(data[4:6], 16))) - 200.0) / (8.0)
            else:
                tamb_degrees = ((((int(data[2:4], 16) << 2) & 0x3C0) + (int(data[4:6], 16) & 0x3F)) - 200.0) / (8.0)
            hamb_percent = (int(data[6:8], 16)) / (2.0)    
        # Create JSON object.
        json_body = [
        {
        ...
        LOG_print_timestamp("[SENSIT] * Monitoring data * site=" + __SENSIT_get_site(sigfox_ep_id) + " vbat=" + str(vbat_mv) + "mV mode=" + str(mode) + " tamb=" + str(tamb_degrees) + "dC hamb=" + str(hamb_percent) + "%")
    # Fill data base.
    if (len(json_body) > 0) :
