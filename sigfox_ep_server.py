...
class SigfoxEpServer:
    ...
    def _update_git_version(self) -> None:
        ...
        try:
            ...
            commit_index = int(subprocess.check_output(["git", "rev-list", "--count", f"{tag}..HEAD"], stderr=subprocess.DEVNULL, text=True).strip())
            # Read commit hash.
            commit_id = subprocess.check_output(["git", "rev-parse", "--short", "HEAD"], stderr=subprocess.DEVNULL, text=True).strip().lower()
            # Check dirty state.
